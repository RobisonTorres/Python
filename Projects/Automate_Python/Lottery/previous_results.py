...
def score():
    ...
    # It also stores the results in a JSON file for future use.
    ...
    for num in range((round - 19), round + 1):
        if str(num) in results.keys():
            ...
            # Extract only the numbers from the scraped text.
    ...
    # Save the updated results dictionary back to the JSON file.
    with open('pre_results.json', 'w') as file:
        json.dump(results, file)
            
    return '\nPrevious Results Saved.'
